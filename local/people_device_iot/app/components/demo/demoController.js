...
app.controller('demoCtrl', ['$scope', 'ConfigService', '$http', '$filter', 'DTOptionsBuilder',
    ...
    function($scope, ConfigService, $http, $filter, DTOptionsBuilder, DTColumnBuilder, $state, $compile, Notification) {
        ...
        document.getElementById('canvas_iframe').onload = function() {
            ...
            // refreshing data
            var refresh_time = 5 * 1000, alive_time = 10;
            inspect_history(); // change this line into desired method
            
            function whole_page(){
                setTimeout(function () { location.reload() }, refresh_time);
            }

            function inspect_history() {
                var inspection_loop = setInterval(function () {
                    change_displaying_mobiles(); // main procedure
                    if ($state.current.name != 'home.demo') clearInterval(inspection_loop);
                }, refresh_time);
            }
            
            function change_displaying_mobiles() { 
                // get a new date value at each interval
                var currently = new Date(), mobile_data
                Object.keys($scope.mobiles).map(function (mobile_id) {
                    // `mobile_data` is like a pointer, changed throughout the loop
                    mobile_data = $scope.mobiles[mobile_id];
                    // calculate the time difference in seconds
                    // test if mobile's lastest data is received within secs ago
                    console.log(currently, '-', mobile_data.time, '=', currently - mobile_data.time)
                    mobile_data.show = ((currently - mobile_data.time) / 1000) <= alive_time;
                    // if not, then use new function from canvas
                    // document.getElementById("canvas_iframe").contentWindow.deleteMobile(mobile_id);
                    // or this simple change;
                    // mobile_data.show = pass_test ? true : false; 
                    updateTable();
                    document.getElementById("canvas_iframe").contentWindow.setMobile(mobile_data);
                })
            }
            // end of refreshing section

            var fuzzy_border_history = {},
                offset = 0.45, lowest_bound = 3;
            var area = [6, 6];
            // 
            var available = function(mobile_id, on_fuzzy_border) {
                var count = 0
                ...
                fuzzy_border_history[mobile_id].push(on_fuzzy_border);                
                return count < lowest_bound || !on_fuzzy_border;
            }
            ...
            socket.on('data', function(msg) {
                ...
                if (typeof($scope.zone) != "undefined") {
                    ...
                    var available_mobile = available(msg.mobile, out_of_box);
                    var mobileItem = {
                        'id': msg.mobile,
                        'col': msg.position[0],
                        'row': msg.position[1],
                    ...
                    // console.log(position_test, "out=" + out_of_box, "show=" + mobileItem.show, mobileItem );
